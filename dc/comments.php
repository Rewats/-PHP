...
function time_elapsed_string($datetime, $full = false) {
    ...
    $diff->d -= $diff->w * 7;
    $string = array('y' => 'yıl', 'm' => 'ay', 'w' => 'hafta', 'd' => 'gün', 'h' => 'saat', 'i' => 'dakika', 's' => 'saniye');
...
function show_comments($comments, $parent_id = -1) {
    ...
    foreach ($comments as $comment) {
        if ($comment['parent_id'] == $parent_id) {
            ...
            $html .= '
            <div class="comment " style=" background-color:white; color:black; border-radius:10px; margin-bottom:20px; padding:20px; box-shadow: 0 4px 25px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);"> 
                <div>
                ...
                <p class=" float-right text-secondary" style="font-size:10px">' . $comment['submit_date'] . '</h6>
            </div>
            ' ;
...
function show_write_comment_form($parent_id = -1) {
    ...
    <div class="write_comment" data-comment-id="' . $parent_id . '" >
        <form>
            ...
            <input name="name" type="text" maxlength="50" placeholder="Ad soyad veya Nick" required>
            <textarea name="content"  placeholder="Yorumun nedir hacı" required></textarea>
...
<div class="comment_header">
    <span class="total"><?=$comments_info['total_comments']?> yorumlar</span>
    <a href="#" class="write_comment_btn" data-comment-id="-1">Yorum Yaz</a>
</div>
